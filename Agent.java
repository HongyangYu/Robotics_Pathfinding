			 ...
			 for (DIR dir : dirs){
				 ...
				 if(between(sx) && between(sy) && !isBlock(map[sy][sx])){
					 ...
					 if(map[sy][sx].getGs() > (es.getGs() + 1)){
						 ...
						 // if succ is not in close list add it to open list
	 ...
	 public void start(int x, int y, int gx, int gy, int[][] maze){
		...
		y_coord.add(start_Y);
		 ...
		 while(!goalTest(cs)){
			 ...
			 OpenHeap.makeEmpty(); // empty open list
			 //empty close list
			 for(int i = 0; i < sizeClosed; i++){
				 Closed[i] = null;
			 }
			 ...
			 cs.setFs();
			 OpenHeap.insert(cs);
			 detect(maze); //detect blocked cell around, and add it. 
			 
			 if(computePath()){
				 // if add new blocked cell then stop moving.
